...
constraint.barV = 20000;
constraint.a = 50;
constraint.beta = constraint.barV/constraint.a;
sim.N = 25;
subsystem{1}.uloc = [1 2];
subsystem{2}.uloc = [3 4];
sim.x0(2) = 0;
%make the centralized MPC matrices
nominal.Bd = [eye(4);zeros(4);zeros(4)];
nominal.nx = 12;
nominal.nu = 4;
nominal.nd = 4;
nominal.N = sim.N;
...
nominal.pmax = 1;
nominal.ws = zeros(nominal.N*4,1);
FLAG = 0;
iter = 0;
while (FLAG==0)
...
while (tsim < sim.time)
  ...
  d = [-0.0001+rand*0.0002;-5+rand*10;-0.0001+rand*0.0002;-0.0001+rand*0.0002];
  x = nominal.A*x + nominal.B*u + [d;d;d];
  %find the warm start
...
data1 = [t(:) results{1}.x(1,:)'+sim.xSS(1) \
	 results{1}.z(1,:)'+sim.xSS(1) \
