...
D1 = kron(eye(N),-model.B);
D2 = kron(diag(ones(N-1,1),-1),-model.A);
D3 = kron(eye(N),eye(size(model.A)));
D = [];
...
mpcmatrices.lhs = kron(eye(N),[eye(nu1) zeros(nu1,nu2) zeros(nu1,nx)]);
...
Dmod1 = kron(eye(N),[zeros(nu2,nu1) eye(nu2) zeros(nu2,nx)]);
...
mpcmatrices = [];
mpcmatrices = mpcmake1(model,penalty,N);
mpcmatrices.lhs = kron(eye(N),[zeros(nu2,nu1) eye(nu2) zeros(nu2,nx)]);
mpcmatrices.lhs = [mpcmatrices.lhs;-mpcmatrices.lhs];
...
Dmod2 = kron(eye(N),[eye(nu1) zeros(nu1,nu2) zeros(nu1,nx)]);
...
mpcmatrices = [];
mpcmatrices = mpcmake1(model,penalty,N);
mpcmatrices.lhs = kron(eye(N),[eye(nu) zeros(nu,nx)]);
mpcmatrices.lhs = [mpcmatrices.lhs;-mpcmatrices.lhs];
...
for k = 1:sim_time
  ...
  if k == 1
    ...
    %steady state input is zeros.
    %Do not share any inputs in decentralized MPC
  endif
  %dynamic MPC
  vecu1_p = vecu1_0;
  vecu2_p = vecu2_0;
